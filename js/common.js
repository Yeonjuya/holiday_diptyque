$(document).ready(function () {
    ...
    $(window).scroll(function () {
        ...
        if (!isMenuOpen) {
            ...
            if (scrollTop > lastScrollTop) {
                ...
                $("#header .util a").css("color", "#232323"); // ✅ 이 줄 추가!
            }
        }
        ...
        if (scrollTop <= 0) {
            $("#header").removeClass("on");
            $("#header .util a").css("color", "#ffffff"); // ✅ 이 줄 추
        }
    ...
    $(function () {
        var $header = $("#header"); // 헤더를 변수에 저장
        var $page = $("#mv .inner"); // 색상이 변할 부분
        var $window = $(window);
        var pageOffsetTop = $page.offset().top; // 색상이 변할 부분의 top값
        ...
        $window.on('scroll', function () { // 스크롤 이벤트
            ...
            if (scrollTop === 0) {
                $header.removeClass('down'); // 스크롤이 0이면 클래스 제거
            } else if (scrollTop >= pageOffsetTop) {
                $header.addClass('down'); // 특정 위치 이상이면 클래스 추가
            }
        });
    });
